#!/usr/bin/env bash
#
# Mint PayBubble S2S + IdAM tokens using the probate caseworker credentials.
#
# Usage:
...
#
# Resolves the probate caseworker username/password from the ccpay-<env> Key
...
if [[ -z "${USER_EMAIL}" || -z "${USER_PASSWORD}" ]]; then
    echo "ERROR: could not fetch probate caseworker credentials from vault '${VAULT}'." >&2
    exit 1
fi
