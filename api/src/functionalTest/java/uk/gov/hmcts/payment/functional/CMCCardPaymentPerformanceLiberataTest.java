...
//         Get card payments liberate pull response time
    ...
    @Test
    public void makeAndRetrieveResponseTime5CardPaymentsByFPLFromLiberata() throws InterruptedException {
        // create Card payments
        final Integer PaymentCount = 30;
        SimpleDateFormat formatter= new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
        ...
        for(int i=0; i<PaymentCount;i++) {
            cardPaymentRequest[i] = PaymentFixture.cardPaymentRequestall("215.00", Service.FPL);
        ...
        Boolean compareResult = new HashSet<>(liberataResponseOld.getPayments()).equals(new HashSet<>(liberataResponseApproach1.getPayments()));
        assertThat(compareResult).isEqualTo(true);
        LOG.info("Comparison of old and new api end point response for 5 card payment is same");
...
//         Get card payments liberate pull response time
        ...
        Response liberataResponseTimeOld = paymentTestService.getLiberatePullPaymentsTimeByStartAndEndDate(SERVICE_TOKEN, startDate,endDate);
        assertThat(liberataResponseTimeOld.statusCode()).isEqualTo(200);
        LOG.info("Response time in milliseconds old api for 5 card payment is : {}",liberataResponseTimeOld.getTime());

        Response liberataResponseTimeApproach1 = paymentTestService.getLiberatePullPaymentsTimeByStartAndEndDateApproach1(SERVICE_TOKEN, startDate,endDate);
        assertThat(liberataResponseTimeApproach1.statusCode()).isEqualTo(200);
        LOG.info("Response time in milliseconds approach 1 api for 5 card payment is : {}",liberataResponseTimeApproach1.getTime());
    }

    @Test
    public void makeAndRetrieveResponseTime5CardPaymentsByUnspecFromLiberata() throws InterruptedException {
        // create Card payments
        final Integer PaymentCount = 30;
        SimpleDateFormat formatter= new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
        ...
        for(int i=0; i<PaymentCount;i++) {
            cardPaymentRequest[i] = PaymentFixture.cardPaymentRequestall("215.00", Service.UNSPEC);
