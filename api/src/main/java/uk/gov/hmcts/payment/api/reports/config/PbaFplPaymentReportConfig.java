...
public class PbaFplPaymentReportConfig implements PaymentReportConfig {
    ...
    private static final String CREDIT_ACCOUNT_PAYMENTS_CSV_FILE_PREFIX = "hmcts_credit_account_payments_fpl_";

    @Value("${pba.fpl.payments.email.from:dummy}")
    private String from;
    @Value("${pba.fpl.payments.email.to:dummy}")
    private String[] to;
    @Value("${pba.fpl.payments.email.subject:dummy}")
    private String subject;
    @Value("${pba.fpl.payments.email.message:dummy}")
    private String message;
    @Value("${pba.fpl.payments.report.scheduler.enabled:false}")
    private boolean enabled;

    @Override
    public PaymentReportType getType() {
        return PaymentReportType.PBA_FPL;
    }
