...
@AllArgsConstructor
@NoArgsConstructor
public class PbaPcsPaymentReportConfig implements PaymentReportConfig<PaymentDto> {
    ...
    private static final String CREDIT_ACCOUNT_PAYMENTS_CSV_FILE_PREFIX = "hmcts_credit_account_payments_pcs_";

    @Value("${pba.pcs.payments.email.from:dummy}")
    private String from;
    @Value("${pba.pcs.payments.email.to:dummy}")
    private String[] to;
    @Value("${pba.pcs.payments.email.subject:dummy}")
    private String subject;
    @Value("${pba.pcs.payments.email.message:dummy}")
    private String message;
    @Value("${pba.pcs.payments.report.scheduler.enabled:false}")
    private boolean enabled;

    @Override
    public PaymentReportType getType() {
        return PaymentReportType.PBA_PCS;
    }
