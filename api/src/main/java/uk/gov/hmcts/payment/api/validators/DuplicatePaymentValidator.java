...
public class DuplicatePaymentValidator {
    ...
    public void checkDuplication(Payment payment, List<PaymentFee> requestFees) {
        ...
        if (!dbPayments.isEmpty()) {
            val sameFees = dbPayments.stream().anyMatch(feePredicate(requestFees));
            val hasSuccessfulPayment = isThereAnySuccessfullyPayment(dbPayments);
            if (sameFees && hasSuccessfulPayment) {
    ...
    private boolean isThereAnySuccessfullyPayment(List<PaymentFeeLink> dbPayment) {
        if (dbPayment == null || dbPayment.isEmpty()) {
            return false;
        }

        return dbPayment.stream()
            .anyMatch(pfl -> pfl.getPayments() != null &&
                pfl.getPayments().stream()
                    .anyMatch(p -> p.getPaymentStatus() != null &&
                        PaymentStatus.SUCCESS.getName().equals(p.getPaymentStatus().getName()))
            );
    }
