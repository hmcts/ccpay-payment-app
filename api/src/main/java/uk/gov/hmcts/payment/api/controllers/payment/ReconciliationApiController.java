package uk.gov.hmcts.payment.api.controllers.payment;
...
@RestController
@Api(tags = {"Payment"})
@SwaggerDefinition(tags = {@Tag(name = "ReconciliationApiController", description = "Payment Reconciliation REST API")})
public class ReconciliationApiController {

    private static final Logger LOG = LoggerFactory.getLogger(ReconciliationApiController.class);
    ...
    @Autowired
    public ReconciliationApiController(PaymentService<PaymentFeeLink, String> paymentService,
        ...
        for (final Payment payment: payments) {
            ...
            if (apportionCheck && apportionFeature) {
                ...
                if(feePayApportionList != null && !feePayApportionList.isEmpty()) {
                    ...
                    fees = new ArrayList<>();
                    paymentService.getApportionedDetails(fees, feePayApportionList);
