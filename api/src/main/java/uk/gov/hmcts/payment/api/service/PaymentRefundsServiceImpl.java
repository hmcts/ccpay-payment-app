        ...
        for(PaymentFee paymentFee : paymentFeeList){
            for (RefundsFeeDto feeDto : paymentRefundRequest.getFees()) {
                ...
                if (feeDto.getId().intValue() == paymentFee.getId().intValue()){
                    ...
                    if(feeDto.getRefundAmount().compareTo(paymentFee.getFeeAmount().multiply(new BigDecimal(feeDto.getUpdatedVolume())))>0 && !(paymentRefundRequest.getRefundReason().equalsIgnoreCase("RR0037"))) {
