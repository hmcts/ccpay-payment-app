...
public class InboundRequestLoggingFilterTest {
    ...
    public void logsRequestAndResponseFields() throws Exception {
        new InboundRequestLoggingFilter(new FakeTicker(20)).doFilterInternal(
                requestFor(GET, "http://localhost/payments/1"),
                responseWithStatus(404),
                mock(FilterChain.class)
        );

        testAppender.assertEvent(0, INFO, "Inbound request start, method: GET, uri: http://localhost/payments/1");
        testAppender.assertEvent(1, INFO, "Inbound request finish, status: 404, elapsedTime: 20");
    }
    ...
    public void logsFailedRequest() throws Exception {
        ...
        testAppender.assertEvent(0, INFO, "Inbound request start, method: GET, uri: http://localhost/payments/1");
        testAppender.assertEvent(1, ERROR, "Inbound request failed, elapsedTime: 20");
    }

    private HttpServletRequest requestFor(HttpMethod method, String url) {
        return new MockHttpServletRequest(method.toString(), url);
    }

    private MockHttpServletResponse responseWithStatus(int status) {
